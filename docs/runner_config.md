...
- `ACTION_ROUND_GROUP_INDEX`：DSL 大组索引（仅在 `battle_runner.q` 手动切换，不在 `battle_config.q` 中配置）。
   ...
   - 3=`Order_Saber.q`
   - 4=`ordeal.q`
- `ACTION_ROUND_INDEX_G0/G1/G2/G3/G4`：各大组内第几套 DSL（从 1 开始，在 `battle_config.q` 配置）。
...
- 文件路径中包含 `battle_config`（包括 `fgo_battle_config`）
- 或命中旧规则 `.../config(...`、`.../config.`

实际编译产物（手机端）通常会出现同一基名的多文件：

- `battle_config(<GUID>).mq`
- `battle_config(<GUID>).prop`
- `battle_config(<GUID>).uis`

说明：
- 其中 runner 只读取 `.mq` 文本内容。
- 文件名中带 GUID（如 `(...)`）是正常现象。
- 只要路径中仍包含 `battle_config`（或命中旧规则），现有扫描逻辑即可识别。

建议：
- 配置脚本命名保持 `battle_config.q`（编译后对应 `battle_config.mq`）。
