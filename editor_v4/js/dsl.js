...
export function generateConfigText(stateData) {
    const rs = stateData.runnerSettings || {};
    const activeGroup = (typeof rs.activeGroup === 'number' && !isNaN(rs.activeGroup)) ? rs.activeGroup : 3;
    const battleCount = (typeof rs.battleCount === 'number' && !isNaN(rs.battleCount) && rs.battleCount > 0) ? rs.battleCount : 30;
    const appleEnable = rs.appleEnable ? 1 : 0;
    const manualChooseFriend = rs.manualChooseFriend ? 1 : 0;
    const forceColorCard = rs.forceColorCard ? 1 : 0;
    ...
    const out = [
        ...
        `Dim ACTION_GROUP_INDEX = ${activeGroup}  ' 当前生效大组: 0=test, 1=campaign, 2=caber, 3=grand, 4=ordeal`,
        `Dim BATTLE_COUNT = ${battleCount}  ' 连续战斗次数`,
        `Dim APPLE_ENABLE = ${appleEnable}  ' 是否吃苹果补充体力 (0=关, 1=开)`,
        `Dim MANUAL_CHOOSE_FRIEND = ${manualChooseFriend}  ' 是否人工选助战 (0=自动找图, 1=人工选择)`,
        `Dim FORCE_COLOR_CARD = ${forceColorCard}  ' 是否强制选择对应色卡 (0=关, 1=开)`,
        "",
        "' ==================== 兼容与兜底字段 ====================",
        `Dim CFG_ACTION_GROUP_INDEX = ${activeGroup}`,
        `Dim MANUAL_BATTLE_COUNT = ${battleCount}`,
        `Dim MANUAL_APPLE_ENABLE = ${appleEnable}`,
        `Dim MANUAL_FORCE_COLOR_CARD = ${forceColorCard}`,
...
export function parseConfigFileText(text, stateData) {
    const getVal = key => {
        ...
        // 关键修复：剥离行尾单引号注释
        const withoutComment = (m[2] || '').split("'")[0].trim();
        return withoutComment;
    };

    stateData.runnerSettings = stateData.runnerSettings || {
        activeGroup: 3,
        battleCount: 30,
        appleEnable: 0,
        manualChooseFriend: 0,
        forceColorCard: 0
    };

    const activeGroupVal = getVal('ACTION_GROUP_INDEX') || getVal('CFG_ACTION_GROUP_INDEX');
    if (activeGroupVal !== '' && !isNaN(Number(activeGroupVal))) {
        stateData.runnerSettings.activeGroup = Number(activeGroupVal);
    }

    const battleCountVal = getVal('BATTLE_COUNT') || getVal('MANUAL_BATTLE_COUNT');
    if (battleCountVal !== '' && !isNaN(Number(battleCountVal))) {
        stateData.runnerSettings.battleCount = Number(battleCountVal);
    }

    const appleEnableVal = getVal('APPLE_ENABLE') || getVal('MANUAL_APPLE_ENABLE');
    if (appleEnableVal !== '' && !isNaN(Number(appleEnableVal))) {
        stateData.runnerSettings.appleEnable = Number(appleEnableVal);
    }

    const chooseFriendVal = getVal('MANUAL_CHOOSE_FRIEND') || getVal('CHOOSE_FRIEND_MANUAL');
    if (chooseFriendVal !== '' && !isNaN(Number(chooseFriendVal))) {
        stateData.runnerSettings.manualChooseFriend = Number(chooseFriendVal);
    }

    const forceColorVal = getVal('FORCE_COLOR_CARD') || getVal('MANUAL_FORCE_COLOR_CARD');
    if (forceColorVal !== '' && !isNaN(Number(forceColorVal))) {
        stateData.runnerSettings.forceColorCard = Number(forceColorVal);
    }
