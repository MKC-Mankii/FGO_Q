...
function initEventBindings() {
    ...
    // 全局运行配置事件绑定 (v4: 5 个原 runner 手工配置)
    const cfgActiveGroup = $('cfgActiveGroup');
    if (cfgActiveGroup) {
        cfgActiveGroup.onchange = e => {
            const val = Number(e.target.value);
            selectGroup(val);
            toast(`🎯 已切换到【大组 ${val}】并设为执行战场`);
        };
    }
    ...
    if (cfgAppleEnable) {
        cfgAppleEnable.onchange = e => {
            if (!appState.data.runnerSettings) appState.data.runnerSettings = {};
            appState.data.runnerSettings.appleEnable = e.target.checked ? 1 : 0;
            renderConfigPreview();
            toast(e.target.checked ? '🍎 已开启自动吃苹果补充体力' : 'ℹ️ 已关闭自动吃苹果');
        };
    }

    const cfgManualFriend = $('cfgManualFriend');
    if (cfgManualFriend) {
        cfgManualFriend.onchange = e => {
            if (!appState.data.runnerSettings) appState.data.runnerSettings = {};
            appState.data.runnerSettings.manualChooseFriend = e.target.checked ? 1 : 0;
            renderConfigPreview();
            toast(e.target.checked ? '🤝 已开启人工选助战（跳过找图）' : 'ℹ️ 已恢复自动识别助战');
        };
    }

    const cfgForceColorCard = $('cfgForceColorCard');
    if (cfgForceColorCard) {
        cfgForceColorCard.onchange = e => {
            if (!appState.data.runnerSettings) appState.data.runnerSettings = {};
            appState.data.runnerSettings.forceColorCard = e.target.checked ? 1 : 0;
            renderConfigPreview();
            toast(e.target.checked ? '🎨 已开启强制对应色卡' : 'ℹ️ 已关闭强制对应色卡');
        };
    }
    ...
    // 模拟器 ADB 指示器点击重新检测并下发同步
    const adbIndicator = $('adbIndicator');
    if (adbIndicator) {
