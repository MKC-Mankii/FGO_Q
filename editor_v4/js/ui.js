...
// 渲染顶部战场 Tabs（v4: 已简化为由顶部「选择战场」下拉框统一切换）
export function renderGroupTabs() {}

// 渲染全局运行配置参数 (v4: 5 个原 runner 手工配置)
export function renderRunnerSettings() {
    const rs = appState.data.runnerSettings || {
        activeGroup: 3,
        battleCount: 30,
        appleEnable: 0,
    ...
    const selGroup = $('cfgActiveGroup');
    if (selGroup) {
        selGroup.innerHTML = appState.data.groups.map((g, idx) => {
            const cleanName = (g.name || `大组 ${idx}`);
            const hint = g.hint ? ` (${g.hint})` : '';
            const isSel = (idx === appState.curGroupIdx);
            return `<option value="${idx}" ${isSel ? 'selected' : ''}>${idx}: ${cleanName}${hint}</option>`;
        }).join('');
    }

    const inputCount = $('cfgBattleCount');
    if (inputCount && document.activeElement !== inputCount) {
        inputCount.value = rs.battleCount ?? 30;
    }

    const chkApple = $('cfgAppleEnable');
    if (chkApple) {
        chkApple.checked = Boolean(rs.appleEnable);
    }

    const chkFriend = $('cfgManualFriend');
    if (chkFriend) {
        chkFriend.checked = Boolean(rs.manualChooseFriend);
    }

    const chkColor = $('cfgForceColorCard');
    if (chkColor) {
        chkColor.checked = Boolean(rs.forceColorCard);
    }
}
